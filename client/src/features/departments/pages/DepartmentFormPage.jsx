...
export default function DepartmentFormPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">HR configuration</p><h1>{editing ? 'Edit department' : 'New department'}</h1><p>{editing ? 'Update the department’s supported details.' : 'Create a department for employee organization and reporting.'}</p></div></header><DepartmentForm department={department} error={error} busy={busy} onSubmit={submit} onCancel={() => navigate('/departments')} /></>
}
