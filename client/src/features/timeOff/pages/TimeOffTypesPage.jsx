...
export default function TimeOffTypesPage() {
  ...
  const submit = async (event) => { event.preventDefault(); setBusy(true); setError(''); const nameError = requiredText(form.name, 'Name'); const normalizedCode = form.code.trim().toUpperCase().replace(/[^A-Z0-9_]/g, ''); const codeError = validateCode(normalizedCode, 'Code'); const descriptionError = optionalText(form.description, 'Description', 2000); if (nameError || codeError || descriptionError) { setError(nameError || codeError || descriptionError); setBusy(false); return } try { const { id, active, createdAt, updatedAt, _id, __v, ...payload } = { ...form, code: normalizedCode, description: form.description.trim() }; void active; void createdAt; void updatedAt; void _id; void __v; if (id) await timeOffTypesApi.update(id, payload); else await timeOffTypesApi.create(payload); setForm(null); await load() } catch (requestError) { setError(getApiError(requestError).message) } finally { setBusy(false) } }
  ...
  return <><header className="page-header"><div><p className="eyebrow">Time Off</p><h1>Time Off Types</h1><p>Configure leave policy; historical policy changes remain backend-protected.</p></div><button className="button" onClick={() => setForm({ ...blank })}>+ New Type</button></header><ErrorBanner message={error} />
    {form && <form className="panel inline-form" onSubmit={submit}><h2>{form.id ? 'Edit' : 'New'} Time Off Type</h2><div className="form-grid"><FormField label="Name *"><input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></FormField><FormField label="Code *"><input required value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, '') })} /></FormField><FormField label="Unit *"><select value={form.unit} onChange={(event) => setForm({ ...form, unit: event.target.value })}><option>DAYS</option><option>HOURS</option></select></FormField><FormField label="Payroll treatment *"><select value={form.payrollTreatment} onChange={(event) => setForm({ ...form, payrollTreatment: event.target.value })}><option>NONE</option><option>PAID</option><option>UNPAID_DEDUCTION</option></select></FormField><FormField label="Description"><textarea rows="3" maxLength={2000} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></FormField><div className="check-grid"><label className="checkbox-row"><input type="checkbox" checked={form.requiresAllocation} onChange={(event) => setForm({ ...form, requiresAllocation: event.target.checked })} /> Requires allocation</label><label className="checkbox-row"><input type="checkbox" checked={form.requiresApproval} onChange={(event) => setForm({ ...form, requiresApproval: event.target.checked })} /> Requires approval</label><label className="checkbox-row"><input type="checkbox" checked={form.isPaid} onChange={(event) => setForm({ ...form, isPaid: event.target.checked })} /> Paid leave</label></div></div><div className="form-actions"><button type="button" className="button button--secondary" onClick={() => setForm(null)}>Cancel</button><button className="button" disabled={busy}>{busy ? 'Saving...' : 'Save Type'}</button></div></form>}
