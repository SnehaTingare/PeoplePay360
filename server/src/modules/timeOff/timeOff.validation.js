...
function typeInput(body, partial = false) {
  ...
  if (!partial || 'code' in body) {
    result.code = typeCode(body.code);
  }
  ...
  if (
    !partial ||
    'payrollTreatment' in body
  ) {
    result.payrollTreatment =
      v.choice(
        body.payrollTreatment,
        [
          'NONE',
          'PAID',
          'UNPAID_DEDUCTION',
...
function allocationInput(
  ...
  for (const key of [
    ...
    if (!partial || key in body) {
      result[key] = v.id(body[key]);
    }
  }

  if (
    !partial ||
    'allocatedAmount' in body
  ) {
...
function requestInput(
  ...
  if (
    ...
    v.invalid(
      'Leave must have positive duration.',
      'LEV-001',
    );
  }
