...
const { comparePassword, hashPassword } = require('../../core/security/password');
...
async function login({ email, password }) {
  ...
  return { token: signAccessToken(user._id), user: userService.serializeUser(user) };
}
...
async function changePassword({ userId, currentPassword, newPassword }) {
  ...
  if (!(await comparePassword(currentPassword, user.passwordHash))) {
    throw appError(errors.AUTH_INVALID_CREDENTIALS, { message: 'Invalid current password.' });
  }
  if (await comparePassword(newPassword, user.passwordHash)) {
    throw appError(errors.USER_INVALID_PASSWORD, {
      message: 'New password must be different from the current password.', statusCode: 422,
    });
  }
