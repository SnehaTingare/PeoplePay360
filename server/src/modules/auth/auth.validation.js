...
function validateChangePassword({ body }) {
  ...
  if (typeof body.currentPassword !== 'string' || !body.currentPassword.length) {
    fail(errors.VALIDATION_ERROR, 'currentPassword', 'Current password is required.');
  }
  if (typeof body.newPassword !== 'string' || body.newPassword.length < PASSWORD_MIN_LENGTH) {
    fail(errors.USER_INVALID_PASSWORD, 'newPassword', `New password must be at least ${PASSWORD_MIN_LENGTH} characters.`);
  }
