...
const object = (value, field = 'body') => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    fail('VALIDATION_ERROR', field, `${field} must be an object.`);
  }
};
...
const allowed = (value, fields) => {
  ...
  if (unknown) {
    fail('VALIDATION_ERROR', unknown, 'Unexpected request field.');
  }
};

const name = value => {
  if (typeof value !== 'string' || !value.trim()) {
    fail('VALIDATION_ERROR', 'name', 'name is required.');
  }
...
function validateWorkingDays(value) {
  if (!Array.isArray(value) || !value.length) {
    fail(
      'SCH-004',
      'workingDays',
      'workingDays must contain at least one schedule line.',
    );
  }
  ...
  return value.map((line, index) => {
    ...
    if (
      !DAYS.includes(line.day) ||
      seen.has(line.day)
    ) {
      fail(
        'SCH-004',
        `${field}.day`,
        'Schedule days must be valid and unique.',
      );
    }

    seen.add(line.day);

    if (typeof line.isWorkingDay !== 'boolean') {
      fail(
        'SCH-004',
        `${field}.isWorkingDay`,
    ...
    if (!line.isWorkingDay) {
      if (
        line.startTime != null ||
        line.endTime != null ||
        ![undefined, 0].includes(line.breakMinutes)
      ) {
        fail(
          'SCH-004',
          field,
          'Non-working days cannot define working times or a break.',
        );
      }
    ...
    if (
      ...
      fail(
        'SCH-002',
        `${field}.breakMinutes`,
        'breakMinutes must be a non-negative number.',
      );
    }
    ...
    if (breakMinutes >= shiftMinutes) {
      fail(
        'SCH-002',
        `${field}.breakMinutes`,
        'breakMinutes must be shorter than the working shift.',
      );
    }
...
function validateList({ query }) {
  ...
  if (query.active !== undefined) {
    if (
      !['true', 'false'].includes(query.active)
    ) {
      fail(
        'VALIDATION_ERROR',
        'active',
...
function validateCreate({ body }) {
  ...
  return {
    body: {
      name: name(body.name),
      workingDays:
        validateWorkingDays(body.workingDays),
    },
  };
}
