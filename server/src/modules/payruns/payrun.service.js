...
const expectedCodes = new Set(['CTR-004', 'CTR-005', 'CTR-006', 'CTR-007', 'PAY-004', 'PAY-005', 'PAY-006', 'PAY-014', 'PAY-015', 'PSL-002']);
const blocking = (employee, code, message, details = {}) => ({ employee, code, severity: 'BLOCKING', message, details });
...
function createPayrunService({
  ...
  async function getPayrun(id) {
    if (!mongoose.isObjectIdOrHexString(id)) throw new AppError('RESOURCE_NOT_FOUND', 'Payrun not found.', 404);
    const record = await Model.findById(id);
    if (!record) throw new AppError('RESOURCE_NOT_FOUND', 'Payrun not found.', 404);
    return record;
  }
  async function loadSelected(ids) {
    const records = await employees.getEmployeesByIds(ids);
    if (records.length !== ids.length) throw new AppError('RESOURCE_NOT_FOUND', 'One or more selected Employees do not exist.', 404);
    return records;
  }
  async function createDraft(input, actor) {
    ...
    await eligibility.assertEmployeesEligible({ employeeList: selected, salaryStructureId: String(scope.salaryStructure._id), periodStart: scope.periodStart, periodEnd: scope.periodEnd });
    return Model.create({ name: input.name, salaryStructure: scope.salaryStructure._id, periodStart: scope.periodStart, periodEnd: scope.periodEnd, employees: input.selectedEmployeeIds, status: PAYRUN_STATUSES.DRAFT, createdBy: actor.id || actor._id });
  }
  async function listPayruns({ status, salaryStructureId, from, to, page, limit }) {
    const filter = {};
  ...
  async function updateDraft(id, input) {
    ...
    if (record.status !== PAYRUN_STATUSES.DRAFT) throw new AppError('RESOURCE_CONFLICT', 'Only a Draft Payrun can be updated.', 409);
    if (input.selectedEmployeeIds) {
      const selected = await loadSelected(input.selectedEmployeeIds);
      await eligibility.assertEmployeesEligible({ employeeList: selected, salaryStructureId: String(record.salaryStructure), periodStart: record.periodStart, periodEnd: record.periodEnd });
  ...
  async function deleteDraft(id) {
    const record = await getPayrun(id);
    if (record.status !== PAYRUN_STATUSES.DRAFT) throw new AppError('RESOURCE_CONFLICT', 'Only a Draft Payrun can be deleted.', 409);
  ...
  function normalizeError(employeeId, error) {
    ...
    if (String(code).startsWith('SAL-') || String(code).startsWith('STR-') || code === 'RESOURCE_NOT_FOUND') code = 'PAY-014';
    if (!expectedCodes.has(code)) return null;
    const messages = { 'PAY-004': 'No applicable contract exists for the selected payroll period.', 'PAY-005': 'Multiple contracts are applicable for the selected payroll period.', 'PAY-014': 'Payroll calculation could not produce a valid result.' };
    return blocking(employeeId, code, messages[code] || error.message, error.details || {});
  }
  function assertComputeAllowed(payrun) {
    if ([PAYRUN_STATUSES.DRAFT, PAYRUN_STATUSES.COMPUTED].includes(payrun.status)) return;
    ...
    if (payrun.status === PAYRUN_STATUSES.VALIDATED) {
      throw new AppError('RESOURCE_CONFLICT', 'Validated Payrun cannot be recomputed.', 409);
    }
  ...
  async function computeEmployee(payrun, employeeId, structure, rules) {
    try {
      ...
      return { success: true, employeeId, snapshot: {
        ...
        employeeSnapshot: { employeeId: employee.employeeId, name: `${employee.firstName} ${employee.lastName}`.trim(), departmentId: employee.department, jobPosition: employee.jobPosition },
        contractSnapshot: { contractId: contract._id, wage: contract.wage, startDate: contract.startDate, endDate: contract.endDate, departmentId: contract.department, jobPosition: contract.jobPosition, workingScheduleId: contract.workingSchedule },
        salaryStructureSnapshot: { salaryStructureId: structure._id, name: structure.name, code: structure.code },
        ...computed, warnings,
      }, warnings };
    } catch (error) {
  ...
  async function computePayrun(id) {
    ...
    for (const result of results) {
      if (result.success) for (const warning of result.warnings) issues.push({ employee: result.employeeId, ...warning });
      else issues.push(result.issue);
    }
    await transaction(async session => {
      for (const result of results) {
        if (result.success) await payslips.upsertComputedPayslip(result.snapshot, session);
        else await payslips.removeUnfinalizedPayslipForEmployee(payrun._id, result.employeeId, session);
      }
    ...
    return { payrun, summary: { successfulPayslips: results.filter(result => result.success).length, blockingEmployees: results.filter(result => !result.success).length, warnings: issues.filter(issue => issue.severity === 'WARNING').length }, issues };
  }
  function assertPayslipSet(payrun, records, requiredStatus, errorCode, message, statusCode = 422) {
    const selected = new Set(payrun.employees.map(String));
    const actual = records.map(record => String(record.employee));
    if (selected.size !== payrun.employees.length || records.length !== payrun.employees.length || new Set(actual).size !== actual.length || actual.some(id => !selected.has(id))) {
  ...
  async function validatePayrun(id, actor) {
    ...
    await transaction(async session => {
      if ((payrun.warnings || []).some(issue => issue.severity === 'BLOCKING')) {
        throw new AppError('PAY-007', 'Blocking payroll issues must be resolved before validation.', 422);
      }
      const records = await payslips.findForPayrun(payrun._id, session);
      assertPayslipSet(payrun, records, PAYRUN_STATUSES.COMPUTED, 'PAY-007', 'Every selected Employee must have exactly one computed Payslip.');
      if (records.some(record => !Number.isFinite(record.netSalary))) {
        throw new AppError('PSL-002', 'Net Salary calculation is missing.', 422, 'BLOCKING');
      }
      await payslips.updateStatusForPayrun({ payrunId: payrun._id, fromStatus: PAYRUN_STATUSES.COMPUTED, toStatus: PAYRUN_STATUSES.VALIDATED, session });
  ...
  async function markPayrunPaid(id, actor) {
    const payrun = await getPayrun(id);
    if (payrun.status !== PAYRUN_STATUSES.VALIDATED) {
      throw new AppError('PAY-010', 'Mark Paid requested before VALIDATED.', 422);
    }
    await transaction(async session => {
      const records = await payslips.findForPayrun(payrun._id, session);
      assertPayslipSet(payrun, records, PAYRUN_STATUSES.VALIDATED, 'PAY-010', 'All Payslips must be validated before the Payrun can be marked paid.');
      await payslips.updateStatusForPayrun({ payrunId: payrun._id, fromStatus: PAYRUN_STATUSES.VALIDATED, toStatus: PAYRUN_STATUSES.PAID, session });
  ...
  async function sendPayslips(id) {
    const payrun = await getPayrun(id);
    if (payrun.status !== PAYRUN_STATUSES.PAID) {
      throw new AppError('RESOURCE_CONFLICT', 'Only Paid Payruns can send Payslips.', 409);
    }
    const records = await payslips.findForPayrun(payrun._id);
    assertPayslipSet(payrun, records, PAYRUN_STATUSES.PAID, 'RESOURCE_CONFLICT', 'Every selected Employee must have exactly one Paid Payslip.', 409);
    const results = [];
    for (const payslip of records) {
      ...
      try {
        ...
        if (!employee.email) {
          results.push({ employeeId: String(payslip.employee), employeeCode: payslip.employeeSnapshot?.employeeId, status: 'FAILED', reason: 'Employee email is missing.' });
          continue;
        }
        ...
        await emails.sendPayslip({ to: employee.email, subject: `Payslip - ${period}`, filename: attachment.filename, pdf: attachment.buffer });
        results.push({ employeeId: String(employee._id), employeeCode: employee.employeeId, status: 'SENT' });
      } catch (error) {
        results.push({ employeeId: String(payslip.employee), employeeCode: employee?.employeeId || payslip.employeeSnapshot?.employeeId, status: 'FAILED', reason: error.code === 'PSL-005' ? 'Payslip PDF generation failed.' : 'Email delivery failed.' });
      }
    }
    return { payrunId: String(payrun._id), sent: results.filter(result => result.status === 'SENT').length, failed: results.filter(result => result.status === 'FAILED').length, results };
  }
  ...
  const listPayslips = id => payslips.listPayslips({ payrunId: id, page: 1, limit: 100 });
  return { createDraft, listPayruns, getPayrun, updateDraft, deleteDraft, computePayrun, validatePayrun, markPayrunPaid, sendPayslips, findForReporting, listPayslips };
}
